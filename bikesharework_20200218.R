...
sum.compare <- ggplot(data = subset(allcities,!is.na(Trip.Duration)), aes(x = City, y = Trip.Duration)) +
...
raw.boxall <- ggplot(data = subset(allcities, !is.na(Trip.Duration)), aes(y = Trip.Duration, color = City)) +
  geom_boxplot() +
...
#Zoom-In on Median
zoom.boxall <- ggplot(data = subset(allcities, !is.na(Trip.Duration)), aes(y = Trip.Duration, color = City)) +
  geom_boxplot() +
...
zoom.histall <- ggplot(data = subset(allcities, !is.na(Trip.Duration)), aes(x = Trip.Duration, color = City)) +
  geom_histogram(binwidth = 60) +
...
zoom.comp <- ggplot(data = subset(allcities, !is.na(Trip.Duration)), aes(x = User.Type, y = Trip.Duration)) +
  geom_boxplot() +
